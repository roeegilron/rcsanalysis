function plot_violin_plots_open_loop_stim_vs_no_stim()
addpath(genpath(fullfile(pwd,'toolboxes','panel-2.14')));
% data dir 
rootdir = '/Users/roee/Starr Lab Dropbox/RC+S Patient Un-Synced Data/database/processed_data';
patientAnalyze = {'RCS02','RCS06','RCS07'};
dataTable = table();
cntTbl = 1;
for p = 1:length(patientAnalyze)
    puse = patientAnalyze{p};
    ff = findFilesBVQX(rootdir,[puse '*psdAndCoherence*stim*.mat']);
    for f = 1:length(ff)
...
for s = 1:size(dataTable,1)
    ...
    idxNum = s;
    chanUse = sprintf('chan%d',str2num(fnuse(4))+1);
    dataTable.ChanUsed{idxNum} = dataStrucStim.database.(chanUse){1};
...
fnsmsv = fullfile(figdirout,'chronic_stim_vs_baseline_openloop_RCS02_RCS06_RCS07.mat'); 
funcUsed = 'plot_violin_plots_open_loop_stim_vs_no_stim.m';
save(fnsmsv,'dataTable','funcUsed');

figdirout = '/Users/roee/Box/rcs paper paper on first five bilateral implants/revision for nature biotechnology/figures/Fig8_effect_chronic_stim';
dataTableLight = dataTable(:,{'patient','side','stim','descriptor','ff','fftOut','fftOutNorm','betapeakuse','meanbetafreq','meanbetafreqNorm','ChanUsed'});
fnsmsv = fullfile(figdirout,'chronic_stim_vs_baseline_openloop_RCS02_RCS06_RCS07_light.mat'); 
funcUsed = 'plot_violin_plots_open_loop_stim_vs_no_stim.m';
save(fnsmsv,'dataTableLight','funcUsed');
