...
%% coherence data table create and load 
% original code 
% do_coherence_on_sep_recordings
resultsdircoherence = '/Users/roee/Starr_Lab_Folder/Data_Analysis/RCS_data/results/at_home/coherence';
ff = findFilesBVQX(resultsdircoherence,'coherenceRes*.mat');
cohResTbl = struct(); 
for f = 1:length(ff)
    load(ff{f},'metaData'); 
    cohResTbl(f).patient = metaData.patient;
    cohResTbl(f).side = metaData.side;
    cohResTbl(f).fileload = ff{f};
    cohResTbl(f).orgfile = metaData.dataFiles;
end
cohResTbl = struct2table(cohResTbl);
...
for dd = 1:length(psdrFiles)
    ...
    close(hfig);
    throwout(dd) = (1- sum(idxkeep)/length(idxkeep))*100;
    ...
    if globalparams.use10minute
        ...
        savefn = sprintf('%s pkg %s.mat',patient{dd},pkgSideUse);
    ...
    sgtitle(sprintf('confriming outlier algo %s',patient{dd}),'FontSize',20);
    close(hfig);
    throwout(dd) = (1- sum(idxkeepcoherence)/length(idxkeepcoherence))*100;
    % confirm that this is a good way to get rid of outliers
    hfig = figure;
    ttls = {'STN 0-2','STN 1-3','M1 8-10','M1 9-11'};
    for c = 1:4
    ...
    if globalparams.use10minute
        ...
        timesPKG = pkgTable.Date_Time;
        timesPKG.TimeZone = 'America/Los_Angeles';
        idxLoop = find(idxkeepcoherence==1);
        
        allDataPkgRcsAccCoh = struct();
        for c = 1:4
            fn = fieldnamesloop{c};
        ...
        makeSepCohrenceStrucutre = 0; 
        if makeSepCohrenceStrucutre
            cnt = 1;
            for i = 1:size(pkgTable,1) % this is looping on rcs data structure
                minTime = timesPKG(i) - minutes(5);
                maxTime = timesPKG(i) + minutes(5);
                ...
                if ~isempty(matchingPsdTimes)
                    ...
                    if duration >= minutes(5) & maxGap < minutes(1)
                        for ccc = 1:4
                            allDataPkgRcsAccCoh.(fieldnamesloop{ccc})(cnt,:)= ...
                                mean(cohResults.(fieldnamesloop{ccc})(:,idxMatch)',1);
                        end
                        
                        allDataPkgRcsAccCoh.timeStart(cnt)  = matchingPsdTimes(1);
                        allDataPkgRcsAccCoh.timeEnd(cnt)  = matchingPsdTimes(end);
                        allDataPkgRcsAccCoh.NumberPSD(cnt) = sum(idxMatch);
                        allDataPkgRcsAccCoh.duration(cnt) = duration;
                        allDataPkgRcsAccCoh.maxgap(cnt) = maxGap;
                        allDataPkgRcsAccCoh.dkVals(cnt,1) = pkgTable.DK(i);
                        allDataPkgRcsAccCoh.bkVals(cnt,1) = pkgTable.BK(i);
                        allDataPkgRcsAccCoh.tremor(cnt,1) = pkgTable.Tremor(i);
                        allDataPkgRcsAccCoh.tremorScore(cnt,1) = pkgTable.Tremor_Score(i);
                        allDataPkgRcsAccCoh.states{cnt} = pkgTable.states{i};
                        allDataPkgRcsAccCoh.numberPkg2minDataPoints{cnt} = cnt;
            ...
            savefn = sprintf('coherence_and_psd %s pkg %s.mat',patient{dd},pkgSideUse);
            fnmsave = fullfile(resultsdir,savefn);
            save(fnmsave,'allDataPkgRcsAcc','cohResults','idxkeepcoherence','psdResults','idxkeep');
