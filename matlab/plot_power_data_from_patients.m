...
powerBandsToPlotNum = input('plot 1 power band (1) or all power bands(2)? '); 
% loop on sides 
uniqueSides = unique(tblPatient.side);

if powerBandsToPlotNum == 2 % plot all power bands 
...
elseif powerBandsToPlotNum == 1 % plot 1 power bands 
    for s = 1:length(uniqueSides)
        idxSide = strcmp(tblPatient.side,uniqueSides{s});
        tblSide = tblPatient(idxSide,:);
        if ~isempty(tblSide)
            ...
            for ss = 1:size(tblSide,1)
                ...
                [combinedDataTable, debugTable, timeDomainSettings,powerSettings,...
                    fftSettings,metaData,stimSettingsOut,stimMetaData,stimLogSettings] = DEMO_ProcessRCS(pn);
                fnsaveCombind = fullfile(pn,'combinedData.mat');
                save(fnsaveCombind,'combinedDataTable','debugTable','timeDomainSettings',...
                    'powerSettings','fftSettings','metaData',...
