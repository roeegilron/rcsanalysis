...
clear all;
close all;
load('/Volumes/RCS_DATA/RCS05/fast_adbs_session2/RCS05R/Session1615412099305/DeviceNPC700415H/AllDataTables.mat');
%%
...
%% Version 1 - use combined data table 
...
hplt = plot(datenum(combinedDataTable.localTime),combinedDataTable.TD_key0); 
xTime = combinedDataTable.localTime;
...
ydet = combinedDataTable.(chanfn);
idxkeep = ~isnan(ydet);
x = combinedDataTable.localTime(idxkeep);
chanfn = sprintf('Adaptive_CurrentProgramAmplitudesInMilliamps');

ycurrent = combinedDataTable.(chanfn)(idxkeep);
ycurrent = cell2mat(ycurrent);
current = ycurrent(:,1);

hsb(cntplt,1) = subplot(nrows,1,cntplt); cntplt = cntplt + 1;
hplt = plot(datenum(x),current,'Color',[0 0.8 0 0.7]); 
row = dataTipTextRow('local time',x);
hplt.DataTipTemplate.DataTipRows(end+1) = row;
hax = hsb(cntplt-1,1);
hax = hsb(cntplt-1,1);
timeStart = dateshift(datetime(datevec(hax.XLim(1))) ,'start','minute');
...
% limits = datetime({'10-Mar-2021 13:58:27','10-Mar-2021 13:58:30'});
% hsb(1,1).XLim = datenum(limits);



%% Version 2 - don't assume chunk is the continous 
hfig = figure;
hfig.Color = 'w';
cntplt = 1;
nrows = 2;
